...
class LoginButton extends StatelessWidget{
  ...
  LoginButton(this.loginController,this.tenantController,this.clientController,this.passwordController);
  @override
        ...
        child: Padding(
          ...
          child: ElevatedButton(
              ...
              onPressed: () async {
                ...
                var response = await sendLogin(req);
                switch (response.statusCode) {
                  ...
                  //Route az instant sale page-re
                    ...
                    box.put('config',new Config(obj.token, obj.clientid, obj.until,
                        obj.dayopened, obj.apiname, obj.storageid,tenant,client));
                    loginController.add(true);
                    return;
                  case 400:
                  //Dialog ha rossz adatokat adtunk meg
                    await showDialog(
                        context: context,
                        builder: (context) {
                          return AlertDialog(
                              content:
                              Text('Érvénytelen felhasználónév vagy jelszó!'));
                        });
                    return;
                  default:
                  //Dialog hiba esetén
                    await showDialog(
                        ...
                        builder: (context) {
                          return AlertDialog(
                              content: Text(
                                  'Hiba történt a bejelentkezés során!${response.body}'));
                        });
                }
              }),
        ),
